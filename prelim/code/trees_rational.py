...
def coverage(document,component):
    ...
    while queue:
        ...
        if record==[-3]:
            ensure(component in ("b2","b4"),"unavailable analytic rule")
            total=Fraction(SCALE+top[0]+top[1],SCALE)
            multiplier=2 if component=="b2" else 4
...
def evaluate_native(component,work):
    filename={"b4":"b4_high_direct.cpp","b6":"b6_high_disjoint.cpp"}[component]
    ...
    with tempfile.TemporaryDirectory(prefix="wilf-rational-") as folder:
        ...
        for response in output:
            ...
            ensure(len(parts)==(3 if component=="b4" else 2)
...
def check(component):
    ...
    if component=="b2":
        specification=importlib.util.spec_from_file_location("b2_direct",BASE/"b2_direct.py")
        ...
        environment=dict(arithmetic="Python integers",source="b2_direct.py")
...
def main():
    ...
    argument_parser.add_argument("component",choices=("b2","b4","b6"))
