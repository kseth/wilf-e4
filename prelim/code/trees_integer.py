...
def root(component):
    return ((Q,Q,{"b2":5,"b4":6,"b6":7}[component]*Q),
            ({"b2":24,"b4":42,"b6":78}[component]*Q,)*3)
...
def coverage(data, component):
    ...
    while stack:
        ...
        if tag==-3:
            require(component!="b6","unsupported analytic leaf")
...
def cpp(component, leaves):
    source=HERE/{"b4":"b4_high_prefix.cpp","b6":"b6_high_subtract.cpp"}[component]
    ...
    with tempfile.TemporaryDirectory(prefix="wilf-integer-") as directory:
        ...
        for row in rows:
            cells=row.split()
            require(len(cells)==(3 if component=="b4" else 2)
...
def check(component):
    ...
    if component=="b2":
        spec=importlib.util.spec_from_file_location("b2_prefix",HERE/"b2_prefix.py")
        ...
        environment=dict(arithmetic="Python integers",source="b2_prefix.py")
...
def main():
    ...
    parser.add_argument("component",choices=("b2","b4","b6"))
