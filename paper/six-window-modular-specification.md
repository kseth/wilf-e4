...
routing cell analytically. B3.3-FV remains conditional for the broader
six-window theorem and is not needed by the retained proof.
...
D3 removed the need to establish it for the main theorem; no R3 replay is
scheduled. Any future use of the broader finite lemma would still require
the full [verification trust policy](../verification/trust-policy.md).

Within this alternative, the modular cuts alone imply the six-window theorem.
The historical real-weight linear programs and their rational dual
certificates are logically redundant. D3 has since removed the entire
alternative from the required verification package.
...
preferred Apéry ideal has at most six final-window points, then
...
The final window is nonempty, so the hypothesis gives \(1\le|Z|\le6\). If
...
> **Dependency decision for this alternative.** Any future modular checker
> need not require the linear-program producer, SciPy, or the stored rational
> duals. They may remain in the immutable archive as diagnostics, but they are
> not part of this arithmetic contract.
...
replay. The source also does not validate every input extraction. Any future
replay must use exact integers or freshly establish and check all fixed-width
bounds, harden malformed-input handling, regenerate the shapes rather than
trust the stored candidate list, and provide the independent
generation-and-lifting path required by V0.

## 7. Superseded alternative interface
...
> preferred Apéry ideal with at most six final-window points satisfies Wilf's
...
The [D3 decision](../research/b3-simplification-decision.md) adopts the
analytic three-plane replacement for the actual \(m\ge30,\ p=(1,1,1)\)
branch. It does not prove B3.3-FV or the broader theorem above; it removes
their necessity and discharges the conditional R3 edge.
