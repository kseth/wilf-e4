...
This is a preliminary computer-assisted proof for independent mathematical
review. All analytic arguments and finite predicates are stated in the
paper. The accompanying verification packet contains the exact computations
used to establish those predicates; no proof-assistant certification is
claimed.

\subsection{Foundational work}

The proof starts from an established model, not a new representation of
numerical semigroups. Zhai \cite{Z} selected preferred factorizations of
Ap\'ery elements and proved a weighted mean inequality for finite lower
ideals. Hellus, Rechenauer, and Waldi \cite{HRW} developed the monomial
initial-ideal and residue-lattice interpretation, including restrictions
on the supports of minimal exclusions. We give the short elementary
arguments needed for our specialization, with attribution at the point
of use. The classical Ap\'ery genus formula supplies the exact link from
...
\cite[Proposition~6.9, published version]{KS}. These are external inputs;
we do not claim to reproduce their computations. Three-dimensional
L-shapes were studied by Aguil\'o-Gost, Garc\'ia-S\'anchez, and Llena
\cite{L}. Chomicz's relation-based construction \cite{C} supplies further
geometric context, discussed in Section~\ref{sec:related}. The clique-tree
ingredient in our geometric decomposition is classical \cite{G}; its
needed finite form is proved here.
...
directly. Integrality matters: a negative Wilf number is at most \(-1\),
not merely a negative real number. For \(m\ge30\), this turns the weakest
surplus into a contradiction.
