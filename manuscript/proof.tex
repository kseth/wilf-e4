...
also called a full-support corner, is a minimal excluded point with all
three coordinates positive.
...
construction of Zhai \cite{Z}. Hellus--Rechenauer--Waldi
\cite[Proposition~2.3]{HRW} identify the monomial ideal complementary to
these lexicographic representatives as the corresponding initial ideal.
...
For
\[
\Lambda=\ker\bigl(\mathbb Z^3\longrightarrow\mathbb Z/m\mathbb Z,
\ x\longmapsto a\cdot x\bmod m\bigr),
\]
the residue bijection gives the disjoint tessellation
\[
\mathbb Z^3=\bigsqcup_{\ell\in\Lambda}(\ell+T),
\]
which is the setting of \cite[Proposition~2.5]{HRW}.
...
\]
Equivalently, Zhai's \cite[Lemma~1]{Z} formula is stated for
\(c'=c-g\), the number of semigroup elements below the conductor.
...
These elementary residue arguments are precisely the support/collision
conclusions of \cite[Proposition~2.6]{HRW}, whose tessellation hypothesis
holds by the preceding display.
...
\]
Here \([\cdot]_m\) denotes the least nonnegative residue. Because the labels
of \(T\) represent every residue modulo \(m\) exactly once,
\[
\sum_{x\in T}[M-\lambda(x)]_m
=\sum_{r=0}^{m-1}r=\binom m2.
\]
Each coordinate direction partitions \(T\), while \eqref{eq:p:2} with the
unnormalized weights \(a\) gives
\[
3mM-4a\cdot s=3\binom m2+\sum_\ell\sigma_\ell.
\]
Since \(m(m-1)=2\binom m2\), equation \eqref{eq:p:1} now yields
\[
...
one line in each direction by the maximality proved above.
...
Choose coordinate \(k\) with \(u_k=v\), and denote the other coordinates
by \(j,\ell\) so that
\[
u_j=\tau,\qquad u_\ell=U,\qquad
0<v\le\tau\le U\le1,qquad \sigma=v+\tau+U.
\]
...
\]
Length-weighted averaging uses \(v\sum L^2/m=2\mu_k+v\), hence, for
\(r\in\{j,\ell\}\),
\[
2\bar\delta_r(1+v+u_r)\ge2u_r\mu_k-v(1+v).
\]
Add the two inequalities, substitute
\(\mu_k=(1+\kappa)/4-\bar\delta_k\) and
\(\bar\delta_\ell=\kappa-\bar\delta_k-\bar\delta_j\), and multiply by two:
\[
\begin{aligned}
(\sigma-v)(1+\kappa)
\le{}&4(1+v+U)\kappa+4v(1+v)\\
&+4(\tau-1-v)\bar\delta_k
-4(U-\tau)\bar\delta_j.
\end{aligned}
\]
Both terms on the second line are nonpositive because the mean slacks are
nonnegative, \(\tau\le1\), and \(\tau\le U\). Discard them and use
\(U\le\sigma-2v\). Rearrangement gives
\begin{equation}
...
Consequently
\[
\sigma<\frac{9v+v^2}{1-3v},\qquad
B=H\sigma<\frac{9H+1}{H-3}=9+\frac{28}{H-3}.
\]
Thus we obtain the necessary failure cutoff
\begin{equation}
\boxed{B<9+28/(H-3).}
\label{eq:p:8}
\end{equation}
...
Permute coordinates and weights together so that
...
For an inclusive box \([x^-,x^+]\), its count is
\(\prod_i(x_i^+-x_i^-+1)\), and twice its objective moment is that count
times \(\sum_i u_i(x_i^-+x_i^+)\). For a clipped box subtract the upper box
\([\max(x^-,p),x^+]\).
The retained feasibility maximum is
\begin{equation}
\max_{\substack{i\\x_i^-\le\min(x_i^+,p_i-1)}}
\left(\ell_i\min(x_i^+,p_i-1)+\sum_{j\ne i}\ell_jx_j^+\right).
\label{eq:p:13}
\end{equation}
...
for \(s_f=0,1,2,3\), these requirements give \(h\ge s_f+3\).
...
\begin{finitelemma}[No-full-corner interval]\label{finite:no-corner-interval}
For all real
...
This is stronger than required for the branch: it requires no coordinate
units, residue labels, exact attainment of \(Q\), or cardinality restriction.
...
There is at most one additional corner at \(x=1\): two planar minimal
exclusions with the same first coordinate would be comparable in the second.
...
A degree-five \(ij\)-plane profile is
\(f^{ij}=(f^{ij}_0,\ldots,f^{ij}_5)\),
\(0\le f^{ij}_r\le6-r\), nonincreasing, encoding
\(\{(r,s):s<f^{ij}_r\}\).
...
For \(xy,xz\), require \(f^{ij}_2\ge2\),
\(|\Delta(f^{ij})|\le2\), and at most one drop at index at least two.
Minimality of \(p\) gives \((1,1,1)=p-e_1\in T\), and lower closure gives
\((0,1,1)\in T\). Thus the \(yz\)-profile satisfies
\(f^{yz}_1\ge2\); also require \(|\Delta(f^{yz})|\le2\).
Use all compatible ordered triples \((f^{xy},f^{xz},f^{yz})\), with
\[
\ell(f^{xy})=\ell(f^{xz}),\quad
f^{xy}_0=\ell(f^{yz}),\quad f^{xz}_0=f^{yz}_0,
\]
and reconstruct
\begin{equation}
T=\{(x,y,z):y<f^{xy}_x,\ z<f^{xz}_x,\ z<f^{yz}_y,
\ (x,y,z)\not\ge(2,1,1)\}.
\label{eq:p:18}
\end{equation}
...
Any extra reconstructed point in an Ap\'ery ideal in this branch would
dominate a planar minimal exclusion, violating its corresponding pair test.
Thus every low-height Ap\'ery ideal in this branch is recovered uniquely.
The profile restrictions do not imply total degree at most five, so the
degree condition remains part of the family.

\begin{finitelemma}[Short-corner low]\label{finite:short-corner-low}
Every such triple, ordered as \((f^{xy},f^{xz},f^{yz})\), satisfies
...
If \(q\) is such a corner, then
\[
r=q-p_ie_i-p_je_j\quad\text{satisfies}\quad
0\le r\le q-e_i\in T.
\]
Here \(p_i,p_j\ge1\), and \(q-e_i\) is included by minimality of \(q\),
so lower closure proves \(r\in T\). This point has residue
\((\gamma+p_k)a_k\);
...
Keep precisely the reconstructions of degree at most six and cardinality at
least 30 that satisfy the three bounds \eqref{eq:p:20}. No maxima,
realizability, modular, or erosion filter is used.
The same plane-recovery and excluded-corner argument as for \eqref{eq:p:18}
proves complete, unique coverage of oriented Ap\'ery ideals in this branch.
...
Remove that exclusion generator to obtain a finite no-full-corner completion:
...
For every \((b,d,Q)\in\mathcal F\), every \(p\in\mathbb N^3\) with all
coordinates positive, \(|p|_1\ge5\), and
\((1,b,d)\cdot p\le Q+1\), and every nonempty finite no-full-corner lower ideal \(U\),
...
\(L=(B_0,C_0,H_0)\), \(V=(B_1,C_1,H_1)\) for \((b,d,Q)\).
First intersect with order:
\[
L\leftarrow(B_0,\max(B_0,C_0),\max(B_0,C_0,H_0)),\quad
V\leftarrow(\min(B_1,C_1,H_1),\min(C_1,H_1),H_1).
\]
...
every relevant real point. Thus twenty iterations give an outward enclosure
whether or not the updates have stabilized.
...
and its integrality gives \(W_4\ge0\).
