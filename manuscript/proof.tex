...
four by Aguiló-Gost--García-Sánchez--Llena \cite{L}. The alternative relation-based
construction of Chomicz \cite{C} provides geometric context but is not a premise.
...
When \(C<0\), \eqref{eq:p:3} gives \(D_Q\ge0>G\); no negative-mass witness is claimed.
...
\]
which is \eqref{eq:p:6}. All inequalities include equality.
...
\subsection{Structural geometry and a reusable exact interval method}\label{sec:p5}
...
The classical graph-theoretic lineage is \cite{G}, but the necessary lemma
is proved in Section~\ref{sec:analytic-a}.
...
restriction. We use them as external theorems; the accompanying packet does not replay
...
\(1\le b\le d\le H\). Equations \eqref{eq:p:8}--\eqref{eq:p:10} give
\[
...
If \(H\ge6\) and \(D_0<m-1\), \eqref{eq:p:8} gives
...
The hypotheses of \eqref{eq:p:22} therefore hold for \(T_z\), including when units
...
Induction proves \eqref{eq:p:26} for the entire real region, not a parameter grid.
...
\(W_4\le-1\), and \eqref{eq:p:1} implies \(AD_0\le m(m-2)\).
...
using the stated, independently replayed finite lemmas and the published
