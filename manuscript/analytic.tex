...
If \(\mathcal T\) has one node, its maximal box is all of \(U\); take that
node as the center and all three horns empty. Hence assume below that
\(\mathcal T\) has at least two nodes.
...
Equivalently, the maximum over proper subrectangles is obtained from the
prefix maxima at \((b-1,d)\) and \((b,d-1)\), because every proper
...
Consequently, any class containing the center-and-horn states of every
relevant ideal gives a valid upper bound, even if it also contains
unrealizable states. Conversely, the recurrences below restrict retained
states only by conditions necessary for feasibility of the exact uncut or
clipped section.
...
Fix the nonempty nested rectangle states \((b_i,c_i)\), \(1\le i\le n\),
in outward order, so \(b_{i+1}\le b_i\) and \(c_{i+1}\le c_i\).
Let state \(i\) occupy the slab from \(t_{i-1}\) to \(t_i\), with
\(t_0=a\). The outward endpoints range over the compact polytope
\[
a=t_0\le t_1\le\cdots\le t_n,
\qquad t_i\le1-b_i-c_i\quad(1\le i\le n).
\]
Endpoint conventions on the slabs do not affect the integral. Put
\(A_i=b_ic_i\) and \(A_{n+1}=0\). Their contribution is
\[
\sum_{i=1}^n A_i
 \int_{t_{i-1}}^{t_i}
 \left(3t+\frac32(b_i+c_i)-2\right)dt.
\]
After like endpoint terms are collected, this is a separable quadratic in
\(t_1,\ldots,t_n\), and the coefficient of \(t_i^2\) is
\(3(A_i-A_{i+1})/2\ge0\). Thus it is convex and has a maximum at a vertex of
the endpoint polytope.

Consider a maximal block of equal endpoints at a vertex. If its common
value is not \(a\) and none of its height inequalities is active, the whole
block can be shifted slightly in either direction while preserving its
order, all strict neighboring gaps, and all height bounds. The vertex would
then be the midpoint of two distinct feasible points, a contradiction.
Hence every block either has value \(a\) or contains an endpoint satisfying
\(t_k=1-b_k-c_k\). Remove the zero-length slabs. If state \(i\) survives,
then the block containing \(t_i\) does not meet the base and starts at \(i\).
For an active \(k\) in that block, nesting and feasibility give
\[
t_i\le1-b_i-c_i\le1-b_k-c_k=t_k=t_i.
\]
Thus every surviving slab is height-tight at its right endpoint. This also
covers coincident height bounds; if no state survives, the horn is empty and
already satisfies the claimed bound.
...
the comparison below applies only to transitions between actual tight
states. A boundary interpolation is a pair of possibly continuous transverse
side functions, nondecreasing in reverse time, that respects the caps and
has side sum \(\rho\) at reverse time \(\rho\). It is used only as a
comparison class for an upper bound; it need not be a finite-step horn.
For a transition from reverse time \(r\) to \(s\), where
\(0\le s\le r\le2/3\), let \(q(\rho)\) be the area of a monotone boundary
interpolation and \(xy\) the smaller endpoint area. Monotonicity gives
\(q(\rho)\ge xy\), while \(A(\rho)\ge A(r)\ge0\). Therefore
\[
...
the boundary between the two states weakly increases the objective.
...
\(A(r)\le0\). In this region lowering a permitted rectangle product weakly
increases its contribution. Write the ordered sides of such a state as
\((x,s-x)\). Nestedness gives \(x\ge u\), and the larger-side cap gives
\(x\ge s-C\). Hence \(x\ge x_*(s)\), where
...
Since \(x(s-x)\) is increasing for \(x\le s/2\), the pair in
\eqref{eq:c:22} has no larger product. It is nested as \(s\) varies, respects
\(B,C\), and ends at \((u,v)\). The nonpositive coefficient in
\eqref{eq:c:21} therefore makes the replacement weakly increase the
...
The possibly slack initial slab has not been interpolated. Set
\(x_0=1-a-C\ge B\), and use the virtual state \((x_0,C)\) only in the formula
for the first jump. It is not an actual rectangle and may exceed the
transverse cap \(B\); the first actual jump absorbs the excess \(x_0-B\).
...
is denoted by \(G(p,q;u,C)\), then
\[
p\ge0,\qquad 0\le q\le C-u.
\]
Fix every contribution before the jump. After its endpoint use the remaining
continuous L-path to \((u,u)\), followed by the balanced tail. Let
\(J_{\mathrm{base}}(p;u,C)\) be the fixed preceding contribution together
with the complete continuous L-path from \((u+p,C)\) through \((u,C)\) and
\((u,u)\), followed by that tail. This baseline has the same fixed start and
tail for every admissible \(q\), so the whole continuation is
\[
J(q)=J_{\mathrm{base}}(p;u,C)+G(p,q;u,C).
\]
...
maximum occurs at \(q=0\) or \(q=C-u\). At \(q=0\) the jump lies entirely
on the fixed-\(C\) branch; at \(q=C-u\) it lands at the balanced state
\((u,u)\).

To compare the latter endpoint, keep the reverse starting time \(r\) fixed.
A jump to \((z,z)\), followed by the balanced tail, has value
...
On the admissible interval \(0\le z\le R\le r/2\), the maximum of \(E_r\)
is therefore at \(0\) or \(R\). For a noninitial crossing jump, take \(R\)
to be the smaller side of its actual starting state. At \(z=R\) the jump has
that side fixed and is dominated by the boundary filling already considered.
At \(z=0\) the suffix vanishes. Every preceding slab has nonpositive
contribution by the choice of \((u,v)\) as the first tight state with
\(u+v\le2/3\), so the entire horn is dominated by the empty horn.
For the initial crossing jump, take \(R=B\), which is valid because
\(2B\le B+C\le1-a\). The endpoint \(z=B\) is the admissible initial slab
ending at \((B,B)\), while \(z=0\) gives the empty horn directly because
there is no preceding contribution. Thus an initial crossing jump either
disappears or becomes an admissible one-slab construction, while a
noninitial one is removed.
...
continuous lengths may be reordered along the same fixed-\(C\) branch before
merging. This preserves the branch endpoints and total length, the fixed
transverse side \(C\) and the other transverse cap, nestedness, the continuous
baseline, and the mandatory initial slack. Equation \eqref{eq:c:25} then
replaces the two adjacent jumps by one with the same total jump length.
Repetition leaves a single initial jump followed by a boundary profile.

If that jump ends at effective caps \((x,y)\), its contribution together with
the optimal continuation is \(F_1(a;x,y)\). Its endpoint is height-feasible,
so \(a+x+y\le1\), and its effective caps satisfy
\(0\le x\le B\), \(0\le y\le C\). Thus it is one of the competitors in
\(U_1(a;B,C)\), and \eqref{eq:c:20} proves the lemma. All comparisons are
weak, so zero-length jumps and equality cases are included.
\end{proof}
