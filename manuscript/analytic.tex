...
The following three arguments are integral to the proof. They require no finite computation.
...
\subsubsection{Exact recurrence interface}
...
additive score \(C_i(t,r,s)\) to the retained section \eqref{eq:a:13} and has a
...
Here \(\lambda(x)\) denotes the unnormalized integer label; the sorted generators are local to this section.
...
from Section~\ref{sec:p3}. That section proves \(E(Z)\ge0\), \(Z\subseteq K\),
and
...
\begin{lemma}[Three columns]\label{lemma:b:4.1}
...
because \(\alpha>\beta\ge1\) implies \(\alpha\ge2\).
\end{proof}

\begin{lemma}[Four columns]\label{lemma:b:4.2}
...
because \(\alpha>\beta>\gamma\ge1\) implies \(\alpha\ge3\).
\end{proof}
...
excludes the six-column triangle, and Lemmas~\ref{lemma:b:4.1}--\ref{lemma:b:4.2} give
\(\Phi(T,K)\ge0\) in the remaining cases. Equation \eqref{eq:b:19} therefore gives
\(\Phi(T,Z)\ge0\), and \eqref{eq:b:18} proves \(W_4(S)\ge0\).

No finite verification is used.
\end{proof}


\subsection{The continuous no-corner moment inequality}\label{sec:analytic-c}
...
objective. Lemma~\ref{lemma:c:3.1} then bounds the filled tail.
...
These follow by integrating \(c(r-c)A(r)\). On the second branch of \eqref{eq:c:23},
...
At most one jump crosses the turn in \eqref{eq:c:23}. Suppose it starts at
...
Where the gain in \eqref{eq:c:12} is active, it is nondecreasing in \(y\). Hence, for
...
larger cap. If \(a+x\ge L/3\), \eqref{eq:c:30} sends the diagonal endpoint to the full
...
\begin{theorem}[Continuous no-corner inequality]\label{theorem:c:5.2}
...
The box side lengths named \(a,b,c\) in this section are local geometric
