...
For soundness, an application must show that every relevant ideal supplies
one of these center-and-horn states; it need not show that every enumerated
state is realizable. Dropping realizability or minimality restrictions only
enlarges the maximum. Any retained-state filter used below is instead proved
as a necessary feasibility condition for the exact uncut or clipped section
being scored.
...
The possibly slack initial slab has not been interpolated. Its position is
represented only for bookkeeping by the virtual state \((x_0,C)\), where
\(x_0=1-a-C\ge B\). This is not an actual rectangle and may exceed the
transverse cap \(B\); the first actual jump records the virtual excess.
