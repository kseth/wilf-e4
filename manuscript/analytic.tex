...
at \(z=0\) the remaining horn is empty. For the initial crossing jump, take
\(R=B\), which is valid because \(2B\le B+C\le1-a\). The endpoint \(z=B\)
is the admissible initial slab ending at \((B,B)\); if \(z=0\) wins, every
earlier term still lies in the nonpositive part, so again the empty horn
dominates. Thus an initial crossing jump either disappears or becomes an
admissible one-slab construction, while a noninitial one is removed.
