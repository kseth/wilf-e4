// Finite Lemma 2.6: recursive profiles; bitset surfaces and two-step masks.
#include <algorithm>
#include <array>
#include <bitset>
#include <cstdint>
...
int main(int argc, char**) {
  try {
    ...
    std::cout<<"{\"component\":\"degree-six-bitsets\",\"status\":\"PASS\",\"complete\":true,\"profiles\":"
             <<pool.size()<<",\"eligible\":"<<total<<"}\n";
    std::cout.flush();need(bool(std::cout),"summary output failure");
  } catch(const std::exception& e) { std::cerr<<"degree-six bitsets failed: "<<e.what()<<"\n";return 1; }
}
