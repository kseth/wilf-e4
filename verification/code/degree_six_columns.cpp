// Finite Lemma 2.6: subset profiles, literal column tops, two-step moves.
#include <algorithm>
#include <array>
#include <bitset>
#include <iostream>
...
int main(int argc,char**) {
 try {
  ...
  std::cout<<"{\"component\":\"degree-six-columns\",\"status\":\"PASS\",\"complete\":true,\"profiles\":"
           <<dictionary.size()<<",\"eligible\":"<<total<<"}\n";
  std::cout.flush();verify(bool(std::cout),"failed output");
 } catch(const std::exception& error) { std::cerr<<"degree-six columns failed: "<<error.what()<<"\n";return 1; }
}
