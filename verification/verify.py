...
def inputs(packet=ROOT):
    ...
    require(type(manifest["schema_version"]) is int and manifest["schema_version"]==1
            and manifest["format"]=="wilf-four-verification-v1","manifest identity")
    ...
    for entry in entries:
        ...
        seen.add(path)
    expected={"README.md","SPECIFICATION.md","verify.py"}
...
def negative_coverage():
    ...
    for name in ("trees_integer","trees_rational"):
        ...
        def document(component,nodes):
            q=4096;r,h={"no-corner-interval":(5,24),"short-corner-high":(6,42),"high-height":(7,78)}[component]
            return dict(schema_version=1,component=component,scale=q,
                        root=[[q,q,r*q],[h*q]*3],nodes=nodes)
        tests=[
            ("unsupported_leaf",document("no-corner-interval",[[-4]]),"no-corner-interval"),
            ("extra_cached_answer",document("no-corner-interval",[[-2,0]]),"no-corner-interval"),
            ("unsound_empty",document("no-corner-interval",[[-1]]),"no-corner-interval"),
            ("unjustified_analytic",document("no-corner-interval",[[-3]]),"no-corner-interval"),
            ("unreachable_node",document("no-corner-interval",[[-2],[-2]]),"no-corner-interval"),
            ("repeated_child",document("no-corner-interval",[[0,5000,1,1],[-2]]),"no-corner-interval"),
            ("boundary_split",document("no-corner-interval",[[0,4096,1,2],[-2],[-2]]),"no-corner-interval"),
            ("boolean_tag",document("no-corner-interval",[[True]]),"no-corner-interval"),
            ("high_analytic",document("high-height",[[-3]]),"high-height"),
            ("wrong_root",document("short-corner-high",[[-2]]),"no-corner-interval"),
        ]
...
def negative_manifest():
    with tempfile.TemporaryDirectory(prefix="wilf-hash-test-") as temporary:
        ...
        source=folder/"SPECIFICATION.md";source.write_text("changed")
        entry=dict(path="SPECIFICATION.md",bytes=7,sha256="0"*64)
        (folder/"manifest.json").write_text(json.dumps(
            dict(schema_version=1,format="wilf-four-verification-v1",inputs=[entry])))
        try: inputs(folder)
...
def run_pair(label,names,validator,native,processes,negatives):
    with tempfile.TemporaryDirectory(prefix="wilf-verification-") as temporary:
        commands=[]
        if native:
...
def replay():
    ...
    record=dict(schema_version=1,packet="wilf-four-verification",status="FAIL",complete=False,started_utc=started,
    ...
    try:
        ...
        for label,names,validate,native in (
            ("generator-box",("generator_box_residues.cpp","generator_box_membership.cpp"),generator_box,True),
            ("short-corner-low",("short_corner_low_offsets.py","short_corner_low_successors.py"),short_corner_low,False),
            ("degree-six",("degree_six_bits.cpp","degree_six_columns.cpp"),degree_six,True),
            ("strip",("strip_prefix.cpp","strip_points.cpp"),strip,True),
        ):
        ...
        for component in ("no-corner-interval","short-corner-high","high-height"):
