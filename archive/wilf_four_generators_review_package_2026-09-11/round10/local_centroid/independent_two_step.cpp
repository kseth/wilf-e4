...
// column-height arrays, direct surface/erosion counts, and a direct local successor checks.
...
bool eligible(const Plane&p,int a,int b,int opp){
  if(p.h[a]<=b)return false;int mixed=0;
  ...
  for(int x=1;x<7;++x)for(int y=1;y<7;++y){
    ...
    if(!here&&left&&below){++mixed;}
  }
  return mixed<=a+b+opp-2;
}
struct Counts{
  uint64_t compatible=0,degree_ok=0,m30=0,q_ok=0,six_maxima=0,remaining=0;
...
int main(int argc,char**argv){
 try{
  const std::string dir=argc>1?argv[1]:"round10/local_centroid";
  const auto planes=make_planes();
  std::ofstream output(dir+"/two_step_verification.jsonl");
  std::ofstream hard(dir+"/cheap_bound_exceptions_strong.jsonl");
  std::ofstream fallback(dir+"/axis_fallbacks_strong.jsonl");
  require(bool(output)&&bool(hard)&&bool(fallback),"cannot open independent outputs");
  output.exceptions(std::ios::failbit|std::ios::badbit);
  hard.exceptions(std::ios::failbit|std::ios::badbit);
  fallback.exceptions(std::ios::failbit|std::ios::badbit);
  ...
  for(int pid=0;pid<(int)corners.size();++pid){
    ...
    for(auto a:lists[0])for(auto b:lists[1]){
      ...
      for(auto c:group[a->ny][b->ny]){
        ++count.compatible;
        int H[9][9]{};int m=0,R=0,Sx=0,Sy=0,Sz=0;bool degree=true;
        ...
        // Every chosen target is at distance zero, one, or two above its line top.
        // The six explicit membership tests cover every coordinate two-step move.
        ...
        for(int x=0;x<7;++x)for(int y=0;y<7;++y)for(int z=0;z<H[x][y];++z){
          ...
          const bool one=!(top[0]&&top[1]&&top[2]);
          const bool two=(z<H[x+2][y]||z<H[x][y+2]||z+2<H[x][y]||
                          z<H[x+1][y+1]||z+1<H[x+1][y]||z+1<H[x][y+1]);
          const int step=two?2:(one?1:0);
          require(!two||one,"lower ideal two-step predecessor");
          if(!one)weighted_maxima+=x+y+z+3;
        ...
        require(un>=0,"strong two-step centroid bound m failed");
      }
    }
    output<<"{\"pid\":"<<pid<<",\"corner\":["<<p[0]<<","<<p[1]<<","<<p[2]<<"]"
      <<",\"compatible\":"<<count.compatible<<",\"degree_ok\":"<<count.degree_ok
      <<",\"m30\":"<<count.m30<<",\"q_ok\":"<<count.q_ok
      <<",\"six_maxima\":"<<count.six_maxima<<",\"remaining\":"<<count.remaining
      <<",\"two_step_upgrade\":"<<count.pure<<",\"axes\":"<<count.axes<<",\"union\":"<<count.united
      ...
      <<",\"max_m_before_surface_filter\":"<<count.max_m<<",\"smallest_tenth_margin\":"<<count.min_unit<<",\"histogram\":{";
    bool first=true;for(int m=30;m<85;++m)if(count.hist[m]){
      if(!first)output<<",";first=false;output<<"\""<<m<<"\":"<<count.hist[m];
    }
    output<<"}}\n";output.flush();hard.flush();fallback.flush();
    std::cerr<<"pid "<<pid<<" verified "<<count.remaining<<" exact closed-form centroid checks; compatible "<<count.compatible<<"\n";
  }

  output.close();hard.close();fallback.close();
  require(total==5574644,"unexpected total remaining shapes");
  std::cout<<"{\"status\":\"passed\",\"profiles\":"<<planes.size()
    <<",\"exact_two_step_centroid_checks\":"<<total<<",\"floating_point_used\":false}\n";
