...
int main(int argc,char**argv){
 ...
 ofstream summary("round10/line_upgrade/coverage_with_failures.jsonl");
 ofstream failure("round10/line_upgrade/upgrade_failures.jsonl");
 for(int x=0;x<7;x++)for(int y=0;y<7;y++)for(int z=0;z<7;z++){
 ...
 for(int pid=0;pid<(int)ps.size();pid++){
  ...
  for(auto a:xy)for(auto b:xz){if(a->hx!=b->hx)continue;
   ...
   for(auto c:zgroups[a->hy][b->hy]){
    ...
    tested++;
    min_gain_margin=min(min_gain_margin,gain-m+2);
    ...
    bool axis=10*axisgainnum>=(10*m-29)*L;
    purepassed+=gain>=m-2;axispassed+=axis;unionpassed+=(gain>=m-2||axis);
    if(gain<m-2){
  ...
  summary<<"{\"pid\":"<<pid<<",\"survivors\":"<<remaining<<",\"pure_upgrade\":"<<purepassed<<",\"axes\":"<<axispassed<<",\"union\":"<<unionpassed<<",\"minimum_gain_minus_m_plus_two\":"<<min_gain_margin<<"}\n";summary.flush();
