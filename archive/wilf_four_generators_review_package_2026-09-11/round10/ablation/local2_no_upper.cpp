...
int main(int argc,char**argv){
 ...
 ofstream summary("round10/ablation/local2_no_upper_coverage.jsonl");
 ofstream failure("round10/ablation/local2_no_upper_first_failure.json");
 for(int x=0;x<7;x++)for(int y=0;y<7;y++)for(int z=0;z<7;z++){
  ...
  for(auto a:xy)for(auto b:xz){if(a->hx!=b->hx)continue;
   ...
   for(auto c:zgroups[a->hy][b->hy]){
    ...
    for(int j=0;j<3;j++)for(size_t id=Fs[j]._Find_first();id<Fs[j].size();id=Fs[j]._Find_next(id)){
      ...
      for(int i=0;i<3;i++){
        auto u=t;u[i]++;
        if(u[i]<=6 && T.test(idx(u[0],u[1],u[2])))local=max(local,1);
        for(int k=i;k<3;k++){
          auto v=u;v[k]++;
          if(v[i]<=6 && v[k]<=6 && T.test(idx(v[0],v[1],v[2])))local=2;
        }
      }
      gain+=(t[j]+1)*local;
    }
