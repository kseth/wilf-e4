...
vector<const Prof*> eligible(int a,int b,int opp){
 ...
 for(auto &p:allp){
  if(p.row[a]<=b)continue; // No total plane-corner restriction.
...
int main(int argc,char**argv){
 ...
 ofstream summary("round10/ablation/no_total_corner_coverage.jsonl");
 ofstream failure("round10/ablation/no_total_corner_first_failure.json");
 for(int x=0;x<7;x++)for(int y=0;y<7;y++)for(int z=0;z<7;z++){
