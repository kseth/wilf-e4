...
vector<const Prof*> eligible(int a,int b,int opp){
 ...
 for(auto &p:allp){
  if(p.row[a]<=b)continue; // No plane-corner count restriction.
  int upper=0;for(auto [x,y]:p.corners)upper+=(x>=a&&y>=b);
  // No dominating plane-corner restriction.
  out.push_back(&p);
 }
 return out;
}
int main(int argc,char**argv){
 ...
 ofstream summary("round10/ablation/only_surface_weak_coverage.jsonl");
 ofstream failure("round10/ablation/only_surface_weak_first_failure.json");
 for(int x=0;x<7;x++)for(int y=0;y<7;y++)for(int z=0;z<7;z++){
