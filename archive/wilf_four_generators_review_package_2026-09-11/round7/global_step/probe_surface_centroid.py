...
for restart in range(10):
 prof=[np.array([N]*N) for _ in range(3)];p=[rng.randrange(1,6) for _ in range(3)]
 cur=evaluate(prof,p)
...
Path(__file__).with_name('surface_centroid_probe.json').write_text(json.dumps(out,indent=2)+'\n')
