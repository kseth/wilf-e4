...
def run(limit=200000,q=4096,hmin=7):
 initial=((q,q,hmin*q),(78*q,78*q,78*q));tree=[None];stack=[(initial,0)];unresolved=[];count=0;start=time.time();dps=0;cases=0
 worker=subprocess.Popen([str(ROOT/'interval_all_corners')],stdin=subprocess.PIPE,stdout=subprocess.PIPE,text=True,bufsize=1)
 while stack and count<limit:
  ...
  if count%10000==0:
   (ROOT/'residual_certificate_checkpoint.json').write_text(json.dumps({'tree':tree,'stack':stack,'unresolved':unresolved,'count':count,'scale':q,'initial':initial}))
 ...
 (ROOT/'residual_interval_certificate.json').write_text(json.dumps(result,separators=(',',':'))+'\n')
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--limit',type=int,default=200000);p.add_argument('--hmin',type=int,default=7);a=p.parse_args();run(a.limit,hmin=a.hmin)
