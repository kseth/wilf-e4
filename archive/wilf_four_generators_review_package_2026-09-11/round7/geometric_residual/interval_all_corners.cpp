...
int main(){
 ...
 I best=std::numeric_limits<I>::min();std::array<int,3>bp={0,0,0};long long count=0;bool stopped=false;
 for(int P=5;P<=int(hi[2]/q)+1&&!stopped;P++)for(int a=1;a<=P-2&&!stopped;a++)for(int b=1;b<=P-a-1&&!stopped;b++){
 int c=P-a-b;
 if(q*a+lo[0]*b+lo[1]*c>hi[2]+q)continue;
 ...
 if(10*v>29*q)stopped=true;
 }
 std::cout<<best<<" "<<bp[0]<<" "<<bp[1]<<" "<<bp[2]<<" "<<count<<" "<<int(!stopped)<<std::endl;
 }
}
