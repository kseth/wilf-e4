...
// Enumerate the full Cartesian range of possible corner coordinates.
// No symmetry pruning is used in this independent replay.
int main() {
    std::ios::sync_with_stdio(false);std::cin.tie(nullptr);
    I scale;Vec lo,hi;
    while(std::cin>>scale>>lo[0]>>lo[1]>>lo[2]>>hi[0]>>hi[1]>>hi[2]) {
        const Vec weights{scale,lo[0],lo[1]};
        const I cap=hi[2]+scale;
        I answer=std::numeric_limits<I>::min(), cases=0;
        Vec witness{0,0,0};
        for(I x=1;weights[0]*x+weights[1]+weights[2]<=cap;x++)
          for(I y=1;weights[0]*x+weights[1]*y+weights[2]<=cap;y++)
            for(I z=1;weights[0]*x+weights[1]*y+weights[2]*z<=cap;z++) {
              if(x+y+z<5)continue;
              Vec p{x,y,z};
              I value=evaluate(scale,lo,hi,p);
              cases++;
              if(value>answer){answer=value;witness=p;}
            }
        std::cout<<answer<<' '<<witness[0]<<' '<<witness[1]<<' '
                 <<witness[2]<<' '<<cases<<'\n'<<std::flush;
    }
}
