...
// rectangle/box moments. Rectangle prefix maxima implement the independently
// audited optimization recurrence. No clipped-box moment formula is used.
#include <algorithm>
#include <array>
#include <iostream>
#include <limits>
...
I solve(I q,std::array<I,3>lo,std::array<I,3>hi,std::array<int,3>p){
 ...
 for(int i=0;i<3;i++){
  ...
  for(int t=n[i]-1;t>=0;t--){
   ...
   // The count, moment and maximum above were computed by summing
   // individual retained points. Only this optimization recurrence uses
   // rectangle prefix maxima; it does not use a clipped-box formula.
   for(int u=0;u<B;u++)for(int v=0;v<C;v++){
    int z=ix(u+1,v+1);I best=0;
    if(mx[z]<=hi[2])best=std::max<I>(0,4*sum[z]+count[z]*(q-3*lo[2])+h(t+1,u,v));
    if(u)best=std::max(best,h(t,u-1,v));
    if(v)best=std::max(best,h(t,u,v-1));
    h(t,u,v)=best;
   }
  }
 }
...
int main(){
 ...
 while(std::cin>>q>>lo[0]>>lo[1]>>lo[2]>>hi[0]>>hi[1]>>hi[2]){
  std::array<I,3>w={q,lo[0],lo[1]};I cap=hi[2]+q;
  I best=std::numeric_limits<I>::min(),cases=0;std::array<int,3>pbest={0,0,0};
  for(int a=1;w[0]*a+w[1]+w[2]<=cap;a++)
   for(int b=1;w[0]*a+w[1]*b+w[2]<=cap;b++)
    for(int c=1;w[0]*a+w[1]*b+w[2]*c<=cap;c++){
     ...
     if(v>best){best=v;pbest=p;}
    }
  std::cout<<best<<' '<<pbest[0]<<' '<<pbest[1]<<' '<<pbest[2]<<' '<<cases<<'\n'<<std::flush;
 }
}
